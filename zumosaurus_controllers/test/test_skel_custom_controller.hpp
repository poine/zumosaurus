...
#ifndef TEST_ZUMOSAURUS_CONTROLLERS_HPP_
#define TEST_ZUMOSAURUS_CONTROLLERS_HPP_
...
#include "zumosaurus_controllers/skel_custom_controller.hpp"
...
class TestableSkelCustomController : public zumosaurus_controllers::SkelCustomController
{
  FRIEND_TEST(SkelCustomControllerTest, joint_names_parameter_not_set);
  FRIEND_TEST(SkelCustomControllerTest, interface_parameter_not_set);
  FRIEND_TEST(SkelCustomControllerTest, all_parameters_set_configure_success);

public:
  CallbackReturn on_configure(const rclcpp_lifecycle::State & previous_state) override
  {
    auto ret = zumosaurus_controllers::SkelCustomController::on_configure(previous_state);
...
class SkelCustomControllerTest : public ::testing::Test
{
public:
  ...
  void SetUp()
  {
    ...
    controller_ = std::make_unique<TestableSkelCustomController>();

    command_publisher_node_ = std::make_shared<rclcpp::Node>("command_publisher");
    command_publisher_ = command_publisher_node_->create_publisher<ControllerCommandMsg>(
      "/test_zumosaurus_controllers/commands", rclcpp::SystemDefaultsQoS());
  }
...
protected:
  ...
  {
    const auto result = controller_->init("test_zumosaurus_controllers");
  ...
  void subscribe_and_get_messages(ControllerStateMsg & msg)
  {
    ...
    auto subscription = test_subscription_node.create_subscription<ControllerStateMsg>(
      "/test_zumosaurus_controllers/state", 10, subs_callback);
...
protected:
  ...
  std::unique_ptr<TestableSkelCustomController> controller_;
...
class SkelCustomControllerTestParameterizedParameters
: public SkelCustomControllerTest,
  public ::testing::WithParamInterface<std::tuple<std::string, rclcpp::ParameterValue>>
{
public:
  virtual void SetUp() { SkelCustomControllerTest::SetUp(); }

  static void TearDownTestCase() { SkelCustomControllerTest::TearDownTestCase(); }

protected:
  void SetUpController(bool set_parameters = true)
  {
    SkelCustomControllerTest::SetUpController(set_parameters);
    controller_->get_node()->set_parameter({std::get<0>(GetParam()), std::get<1>(GetParam())});
  }
};

#endif  // TEST_ZUMOSAURUS_CONTROLLERS_HPP_
